...
void pubsub::joy_callback(const sensor_msgs::msg::Joy::SharedPtr msg)
  {  
    ...
    //右回転
    publisher_->publish(get_frame(0x101, 6.28f*(2*x+r)));
    publisher_->publish(get_frame(0x111, 6.28f*(x-y*static_cast<float>(sqrt(3))+r)));
    publisher_->publish(get_frame(0x121, 6.28f*(x-y*static_cast<float>(sqrt(3))+r)));
